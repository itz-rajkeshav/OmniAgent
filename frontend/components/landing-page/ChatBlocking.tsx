...
import { Lock, Unlock, Sparkles, User, Users } from "lucide-react";
      ...
      <div className="container mx-auto px-4 max-w-6xl relative z-10 text-center">
        ...
        >
          ...
          <p className="text-zinc-600 text-lg md:text-xl leading-relaxed">
            Block specific chats or entire groups from AI replies. OmniAgent will never respond in conversations you mark private — your family group, close friends, or any thread you want left alone.
          </p>
        </motion.div>
        ...
        >
          <div className="bg-white rounded-2xl p-6 shadow-xl border border-zinc-100">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-zinc-100">
              <h3 className="font-outfit font-bold text-xl text-zinc-900">Active Conversations</h3>
              <span className="text-xs font-medium px-3 py-1 bg-brand-accent-blue/10 rounded-full text-brand-accent-blue">Tap to block</span>
            </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        chat.locked ? "bg-zinc-200 text-zinc-500" : "bg-brand-accent-blue/10 text-brand-accent-blue"
                      }`}>
                        {chat.type === "group" ? <Users className="w-5 h-5" /> : <User className="w-5 h-5" />}
                      </div>
                      <div className="text-left">
                        <div className={`font-medium ${chat.locked ? "text-zinc-400 line-through decoration-zinc-300" : "text-zinc-900"}`}>
                          {chat.name}
                        </div>
                        <div className="text-xs mt-0.5">
                          {chat.locked ? <span className="text-zinc-500">AI paused</span> : <span className="text-brand-accent-emerald font-medium">AI active</span>}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
                      ...
                      >
                        {chat.locked ? <Lock className="w-5 h-5" /> : <Sparkles className="w-5 h-5" />}
                      </motion.div>
                    </div>
                  </motion.div>
                ))}
            ...
            <p className="text-sm text-zinc-400 mt-6 font-inter italic">
              * Blocklist persists across devices and sessions. Your rules are permanent.
            </p>
          </div>
        </motion.div>
      </div>
