...
export function BlockedContactsClient({ userId }: { userId: string }) {
  ...
  const [initialBlockedJids, setInitialBlockedJids] = useState<Set<string>>(new Set());
  ...
  const handleSave = async () => {
    ...
    try {
      const toBlock = Array.from(blockedJids).filter(jid => !initialBlockedJids.has(jid));
      const toUnblock = Array.from(initialBlockedJids).filter(jid => !blockedJids.has(jid));
      ...
      const promises = [
        ...toBlock.map(jid => fetch(`${GATEWAY_URL}/whatshapp/block`, reqOptions(jid))),
        ...toUnblock.map(jid => fetch(`${GATEWAY_URL}/whatshapp/unblock`, reqOptions(jid)))
      ];
  ...
  const filteredItems = useMemo(() => {
    ...
    // Sort items so blocked items are always at the top
    return [...list].sort((a, b) => {
      const aBlocked = blockedJids.has(a.jid);
      const bBlocked = blockedJids.has(b.jid);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-20">
        ...
        <p className="text-[#667781] font-medium">Loading contacts and groups...</p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-3xl p-8 border border-[#E9EDEF] shadow-sm mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold font-outfit text-[#111B21] tracking-tight">Access Control</h1>
              <p className="text-[#667781] text-lg">Select users or groups to prevent the agent from replying.</p>
            </div>
          </div>
        ...
        <div className="space-y-3">
          {uniqueItems.length === 0 ? (
            <div className="text-center py-16 border-2 border-dashed border-[#E9EDEF] rounded-2xl">
              <p className="text-[#8696A0] font-medium text-lg">No {activeTab} found</p>
            </div>
          ) : (
            <AnimatePresence>
              {uniqueItems.map((item) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all duration-300 ${isBlocked ? "bg-red-50 border-red-200 shadow-sm" : "bg-white border-[#E9EDEF] hover:border-[#128C7E] hover:shadow-sm"}`}
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${isBlocked ? "bg-red-100 text-red-600" : "bg-[#F0F2F5] text-[#54656F]"}`}>
                        {item.name ? item.name.charAt(0).toUpperCase() : (item.isGroup ? "G" : "U")}
                      </div>
                      <div>
                        <h3 className={`font-bold ${isBlocked ? "text-red-900" : "text-[#111B21]"}`}>
                          {item.name || "Unknown"}
                        </h3>
                        {item.phoneNumber && (
                          <p className={`text-sm font-medium ${isBlocked ? "text-red-700/70" : "text-[#667781]"}`}>
                            +{item.phoneNumber}
                          </p>
                        )}
                    ...
                    <div className="flex items-center">
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${isBlocked ? "bg-red-500 border-red-500 scale-110" : "border-[#AEB9D1]"}`}>
                        {isBlocked && <UserX className="w-3.5 h-3.5 text-white" />}
                      </div>
                    </div>
                  </motion.div>
                );
