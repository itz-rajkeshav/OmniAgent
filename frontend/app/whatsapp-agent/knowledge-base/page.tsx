...
export default function KnowledgeBasePage() {
  ...
  const fetchSources = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || data.status === "error") {
        throw new Error(
          data.message || data.detail || "Failed to fetch sources",
        );
      }
      const formatted: SourceItem[] = (data.sources || []).map(
        (source: any) => ({
  ...
  const removeSource = async (source: SourceItem) => {
    ...
    setIsLoading(true);
    setLoadingMessage("Deleting source...");
    setBannerMsg(null);
    ...
    try {
      ...
      if (!res.ok || data.status === "error") {
        throw new Error(
          data.message || data.detail || "Failed to delete source",
        );
      }
  ...
  return (
    <>
      {isLoading && (
        <div className="fixed inset-0 z-50 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center gap-3 text-center px-6">
          <RefreshCw className="w-8 h-8 animate-spin text-indigo-600" />
          <p className="font-bold text-[#111B21] text-lg">
            {loadingMessage || "Working on your knowledge base..."}
          </p>
          <p className="text-sm text-[#667781] font-medium">
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-3xl p-8 border border-[#E9EDEF] shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-[#111B21] tracking-tight">
                Knowledge Base
              </h1>
              <p className="text-[#667781] mt-1.5 text-base font-medium">
                Train your WhatsApp Agent with custom documents and web URLs.
              </p>
            </div>
          </div>
                ...
                >
                  ...
                  {sources.length === 0 ? (
                    <div className="text-center py-20 border-2 border-dashed border-[#E9EDEF] rounded-[2rem] bg-gray-50/50">
                      ...
                      <h3 className="text-2xl font-extrabold text-[#111B21] mb-3">
                        No data sources yet
                      </h3>
                      <p className="text-[#667781] mb-8 max-w-md mx-auto text-lg">
                        Add PDF files or text snippets so the agent can learn
                        about your business and answer customer queries
                        accurately.
                      </p>
                            ...
                            >
                              <td className="py-4 pl-4">
                                <div className="flex items-center gap-4">
                                  ...
                                  <span
                                    className="font-bold text-[#111B21] text-base truncate max-w-[200px] sm:max-w-[260px] block"
                                    title={source.name}
                                  >
                                    {source.name}
                                  </span>
                              ...
                              <td className="py-4 font-medium text-[#667781]">
                                {source.date}
                              </td>
              ...
              {activeTab === "add-pdf" && (
                ...
                >
                  <div className="border-2 border-dashed border-gray-300 hover:border-indigo-400 bg-gray-50/50 hover:bg-indigo-50/30 rounded-[2.5rem] p-16 text-center transition-all cursor-pointer group shadow-sm">
                    ...
                    {pdfFile ? (
                      <div>
                        <h3 className="text-xl font-bold text-[#111B21] mb-2">
                          {pdfFile.name}
                        </h3>
              ...
              {activeTab === "add-website" && (
                ...
                >
                  <div className="bg-white rounded-[2.5rem] p-10 border border-gray-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    <div className="mb-8">
                      <label className="block text-base font-bold text-[#111B21] mb-3">
                        Enter Website URL
                      </label>
                      ...
                      <p className="mt-3 text-sm font-medium text-gray-500 pl-1">
                        Our scraper will extract written content to enrich your
                    ...
                    <div className="flex items-start gap-4 mb-10 bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100">
                      <div className="mt-1">
                        <input
                          ...
                          onChange={(e) =>
                            setIsCrawlingSubpages(e.target.checked)
                          }
                      ...
                      <div>
                        <label
                          ...
                          className="font-bold text-lg text-indigo-950 block cursor-pointer"
                        >
                          Crawl related sub-pages deeper
                        </label>
                        <span className="text-sm font-medium text-indigo-700/80 mt-1 block">
                          Extracts more comprehensive data, but may take several
                          minutes depending on the website's structure and size.
                        </span>
                      </div>
                    </div>
